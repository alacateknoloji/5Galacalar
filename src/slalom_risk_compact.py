#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Compact vehicle slalom/speed detection module."""
...
SHOW_VIDEO = True
VEHICLE_LABELS = ["ambulans", "hatchback", "itfaiye", "kamyon", "minibus", "panelvan", "pickup", "sedan", "suv"]


def resolve_model_path(models_dir=None):
    """Resolve the shared vehicle-detection weight used by the pipeline."""
    candidates = [models_dir, os.environ.get("MODELS_DIR"), os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir)), "models"), "/app/models"]
    for candidate in candidates:
        if not candidate or not os.path.isdir(candidate):
            continue
        base = os.path.join(candidate, vehicle_type.WEIGHT_FILE)
        for path in utils._candidate_weight_paths(base):
...
class VehicleTracker:
    def __init__(self, max_distance=MAX_TRACK_DISTANCE, max_history_seconds=MAX_HISTORY_SECONDS, vehicle_labels=None):
    ...
    def update(self, detections, frame_index, fps):
        dets = [
            {"label": d["label"], "bbox": d["bbox"], "confidence": float(d.get("confidence", 0.0)), "center": get_center(d["bbox"])}
        ...
        for di, det in enumerate(dets):
            if di in assignment:
                ...
                track.update({"label": det["label"], "bbox": det["bbox"], "confidence": det["confidence"], "center": det["center"], "last_frame": frame_index})
...
class SlalomRiskAnalyzer:
    ...
    def analyze(self, track):
        ...
        zigzag_count = self._count_direction_changes(xs)
...
class SpeedEstimator:
    def estimate(self, track, fps):
        ...
        if len(recent) < 2:
            return 0.0
        total_px = sum(math.hypot(recent[i][1] - recent[i - 1][1], recent[i][2] - recent[i - 1][2]) for i in range(1, len(recent)))
...
class SlalomDetectionSystem:
    def __init__(self, max_distance=MAX_TRACK_DISTANCE, min_lateral=MIN_LATERAL_MOVEMENT, zigzag_threshold=ZIGZAG_THRESHOLD, vehicle_labels=None):
    ...
    def process_frame(self, detections, frame_index, fps):
        ...
        for track in tracks:
            ...
            all_tracks.append({"track_id": track["track_id"], "label": track["label"], "bbox": [int(round(v)) for v in track["bbox"]], "center": [int(round(track["center"][0])), int(round(track["center"][1]))], "speed_kmh": speed_kmh, "speeding": speeding, "slalom": res["slalom"]})
            if res["slalom"]:
                slalom_detections.append({"track_id": track["track_id"], "label": track["label"], "center": [int(round(track["center"][0])), int(round(track["center"][1]))], "slalom": True, "reason": res["reason"], "confidence": res["confidence"], "speed_kmh": speed_kmh})
...
def _collect_sources(source):
    ...
    if os.path.isdir(source):
        ...
        files = sorted({path for ext in exts for path in glob.glob(os.path.join(source, f"*.{ext}")) | glob.glob(os.path.join(source, f"*.{ext.upper()}"))}, key=_natural_key)
        return files, "images"
    return [source], "video"
...
def _draw_annotations(frame, out):
    ...
    for t in out.get("all_tracks", []):
        ...
        speed_label = f"{t['label']} #{t['track_id']} | {t['speed_kmh']} km/h"
        cv2.putText(frame, speed_label, (x1, max(y1 - 22, 0)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 1, cv2.LINE_AA)
        warnings = []
        if t.get("slalom"):
        ...
        if warnings:
            cv2.putText(frame, " | ".join(warnings), (x1, max(y1 - 5, 0)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
    return frame
...
def _print_frame_info(out, frame_index):
    for t in out.get("all_tracks", []):
        ...
        print(f"[KARE {frame_index}] #{t['track_id']} {t['label']:12s} {t['speed_kmh']} km/h{flag_str}")
...
def run_inference(model_path=None, source=SOURCE, fps=VIDEO_FPS, conf=CONF_THRESHOLD, label_map=None):
    ...
    if kind == "images" and len(items) == 1:
        print("UYARI: Tek bir goruntu verdin. Slalom zamansal bir davranistir; tek kareden slalom uretilemez.")

    outputs = []
    for frame_index, result in enumerate(model(items[0], stream=True, conf=conf, verbose=False) if kind == "video" else [model(path, conf=conf, verbose=False)[0] for path in items]):
        ...
        if cv2_ok and SHOW_VIDEO:
            ...
            frame = _draw_annotations(frame, out)
            cv2.imshow("Slalom + Hiz Takibi", frame)
...
if __name__ == "__main__":
    outputs = run_inference(model_path=MODEL_PATH, source=SOURCE, fps=VIDEO_FPS, conf=CONF_THRESHOLD, label_map=LABEL_MAP or None)
